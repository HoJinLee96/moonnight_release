...
public class VerificationService {
	...
	public int sendSmsVerificationCode(String recipientPhone, String clientIp) {
		...
		VerificationBuilder verificationBuilder = Verification.builder().clientIp(clientIp).recipient(recipientPhone)
		...
		if ((sendStatus / 100) != 2) {
			log.error("* 인증번호 발송 요청 응답코드 실패: SendStatus: [{}], RecipientPhone: [{}] , ClientIp: [{}]", sendStatus,
					recipientPhone, clientIp);
	...
	public int sendEmailVerificationCode(String recipientEmail, String clientIp) {
		...
		VerificationBuilder verificationBuilder = Verification.builder().clientIp(clientIp).recipient(recipientEmail)
				.verificationCode(verificationCode);
		int sendStatus = 0;
		try {
		...
		if ((sendStatus / 100) != 2) {
			log.error("* 인증번호 발송 요청 응답코드 실패: SendStatus: [{}], RecipientEmail: [{}] , ClientIp: [{}]", sendStatus,
					recipientEmail, clientIp);
	...
	public String compareSms(String encodedVerificationId, String phone, String reqCode, String clientIp) {
		...
		log.debug("*문자 인증번호 검증. VerificationId: [{}], ClientIp: [{}]",
				LogMaskingUtil.maskId(verificationId, MaskLevel.MEDIUM), clientIp);

		compareCode(verificationId, phone, reqCode, clientIp);

		return tokenProvider.createToken(new VerificationPhoneTokenDto(verificationId + "", phone),
				VerificationPhoneTokenDto.TOKENTYPE);
	}
	...
	public String compareEmail(String encodedVerificationId, String email, String reqCode, String clientIp) {
		...
		return tokenProvider.createToken(new VerificationEmailTokenDto(verificationId + "", email),
				VerificationEmailTokenDto.TOKENTYPE);
	}

	/**
	 * 주어진 인증 ID가 유효하게 처리되었는지 검증합니다.
	 * 
	 * DB에서 인증 요청 정보를 조회하여, 인증이 완료되었는지, 그리고 미인증 상태라면 유효 시간이 만료되지는 않았는지 확인합니다.
	 *
	 * @param verificationId 검증할 인증 요청의 고유 ID
	 * 
	 * @throws NoSuchDataException          DB에서 해당 ID의 인증 요청을 찾을 수 없는 경우 {@link #isVerify}
	 * @throws VerificationExpiredException 인증은 되었으나 유효 시간이 만료된 경우 {@link #isVerify}
	 * @throws NotVerifyException           아직 인증이 완료되지 않은 요청인 경우 {@link #isVerify}
	 * 
	...
	public void isVerify(int verificationId) {
		...
		VerificationProjection verificationProjection = verificationRepository
				.findVerificationWithValidity(verificationId)
